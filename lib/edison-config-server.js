...
sensorpid.on('exit', function (exitCode) {
...
function findMongo(){
 ...
 var errmsg = "Can't find mongod";
  console.log("Finding Mongo with: " + exec_cmd);
...
// Listen for an exit event:
mongopid.on('exit', function (exitCode) {
    console.log("Child exited with code: " + exitCode);
});
...
function runCmd(i, commands) {
  ...
  if (commands[i].cmd && !WHITELIST_EXEC[commands[i].cmd]) {
    console.log("Command " + commands[i].cmd + " not in WhiteList!");
    return;
  }

 console.log("Executing " + commands[i].cmd + " " + commands[i].args[0] + "\n");
  ...
  commands[i].proc.on('error', function (err) {
    console.log(commands[i].cmd + " " + commands[i].args[0] +
...
function submitForm(params, res, req) {
  ...
  console.log("submitForm params: " + JSON.stringify(params, null, 4));
  var calls = [setPass, setHost, doSleep, setWiFi, findMongo];
  var result = null, commands = [];

   if (params.data == "findMongo") {
        console.log("short circuit to mongo ... ");
	findMongo();
        return;
    }
   if(params.data == "resetSensors"){
	resetSensors();
	return;
   }
...
function handlePostRequest(req, res) {
  console.log("handlePost: " + urlobj.pathname);
...
function inWhiteList(path) {
  ...
  console.log("inWhiteList: " + result);
...
function requestHandler(req, res) {
  ...
  console.log("Load: " + urlobj.pathname);
  if (!inWhiteList(urlobj.pathname)) {
    console.log(" NOPE! Load: " + urlobj.pathname);
    pageNotFound(res);
    return;
  }
